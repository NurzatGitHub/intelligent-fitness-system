...
from ai_model.scripts.dataset_extractor_squat import build_dataset
...
MODEL_PATH = os.path.join(MODELS_DIR, "squat_model.pkl")
...
def train_and_save():
    ...
    if len(classes) < 2:
        raise RuntimeError(f"Dataset has only one class: {classes}. Fix extraction before training.")
